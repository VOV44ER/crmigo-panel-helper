...
serve(async (req) => {
  ...
  try {
    ...
    const url = new URL('https://api.publisher.tonic.com/v4/keywords/stats');
    if (from) url.searchParams.append('from', from);
    if (to) url.searchParams.append('to', to);
    if (username) url.searchParams.append('campaignName', username);
    if (countryCodes) url.searchParams.append('countryCodes', countryCodes);
    ...
    console.log('Successfully fetched keywords stats');
